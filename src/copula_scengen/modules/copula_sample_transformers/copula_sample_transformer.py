...
class CopulaSampleTransformer(CopulaSampleTransformationStrategy):
    def _margin_types(self, data: pd.DataFrame) -> dict[int, MarginType]:
        margin_types = {}
    ...
    def _compute_transformations(self, data: pd.DataFrame, n_scenarios: int) -> np.ndarray:
        margin_transformations = np.zeros((data.shape[1], n_scenarios), dtype=float)
        margin_types = self._margin_types(data=data)

        ranks = np.arange(1, n_scenarios + 1)
        for margin_index in range(data.shape[1]):
            if margin_types[margin_index] == MarginType.DISCRETE:
                margin_data = data.iloc[:, margin_index].to_numpy()
    ...
    def transform(self, data: pd.DataFrame, copula_sample: CopulaSample) -> pd.DataFrame:
        transformations = self._compute_transformations(data=data, n_scenarios=copula_sample.max_rank)
